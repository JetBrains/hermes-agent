...
class TestSlackProxyBehavior:
    ...
    async def test_before_authorize_clears_env_proxy_per_request(self, monkeypatch):
        ...
        monkeypatch.setenv("NO_PROXY", "127.0.0.1,localhost,slack.com")

        app_client = _slack_mod.AsyncWebClient(token="xoxb-fake")
        _slack_mod._apply_slack_proxy(app_client, None)
        _slack_mod._apply_slack_base_url(app_client, "http://127.0.0.1:65535/api/")
        ...
        # still talks to the configured endpoint.
        assert request.context.client.base_url == "http://127.0.0.1:65535/api/"
...
class TestSlackBaseUrl:
    ...
    async def test_connect_applies_custom_base_url_to_clients(self):
        ...
        class FakeApp:
            # The adapter builds the app with an explicit ``client=`` (a
            # pre-built AsyncWebClient carrying the Hermes user-agent prefix).
            def __init__(self, token=None, client=None, **_kwargs):
                self.token = token
                self.client = client if client is not None else FakeWebClient(token)
                created_apps.append(self)
