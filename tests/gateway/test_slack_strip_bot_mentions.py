...
class TestThreadContext:
    ...
    @pytest.mark.parametrize(
        "strip,expected",
        [(True, "hey  look at this"), (False, "hey @TestBot look at this")],
    )
    async def test_block_kit_mention_is_not_duplicated(self, strip, expected):
        """Block dedupe compares against the pre-render text.

        Both sides are normalized by deleting ``<@bot_uid>``, so feeding the
        rendered ``@BotName`` would never match the block's raw token and the
        authored message would be appended a second time.
        """
        adapter = delivery_adapter(strip=strip)
        parent = {
            "ts": "100.000",
        ...
        assert parent_text == expected.strip()
