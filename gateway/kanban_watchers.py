            ...
            try:
                ...
                for d in deliveries:
                    ...
                    for ev in d["events"]:
                        ...
                        # A "wake"-only subscription has no passive send to
                        # attempt, so the non-push guard does not apply to it.
                        if send_passive and not adapter_supports_push(adapter):
                            logger.debug(
                        ...
                        try:
                            if send_passive:
                                ...
                                # A SendResult(success=False) without an
                                # exception (returned by push-capable adapters
                                # on a genuine transient failure) must count as
                                # a FAILED delivery — otherwise the cursor
                                # advances and the event is permanently lost.
                                # Adapters returning None (or anything
