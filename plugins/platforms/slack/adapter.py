...
def _slack_message_body_signature(message: dict) -> str:
    ...
    parts = [text]
    if blocks:
        parts.append(_extract_additional_text_from_slack_blocks(blocks, text))
...
def _slack_per_request_proxy_middleware(
    ...
    request-scoped client exists and before that first call. ``base_url`` needs
    no pinning: Bolt copies it off ``app.client`` per request, where
    ``_apply_slack_base_url`` has already applied it.
    """
...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def _handle_slack_message(
        ...
        if blocks and not is_command_text:
            # The dedupe deletes the bot token from both sides to bridge the
            # strip applied further down; when the mention is kept, both sides
            # are raw already and deleting it would empty a mention-only block,
            # which then reads as new content and gets appended.
            if self._slack_strip_bot_mentions():
                dedupe_bot_uid = (
                    self._team_bot_user_ids.get(dedup_team_id, self._bot_user_id)
                    or ""
                )
            else:
                dedupe_bot_uid = ""
    ...
    def _render_message_text(
        ...
        ``@bot_name`` instead of deleted. Block dedupe is then fed the raw text
        with no ``bot_uid``, so both sides of the comparison carry the token as
        written — otherwise the authored message is appended a second time.
        """
        ...
        if blocks:
            rich_text = _extract_additional_text_from_slack_blocks(
                blocks, raw_text, bot_uid=bot_uid if strip_bot_mention else ""
            ).strip()
            if rich_text:
    ...
    async def _fetch_thread_parent_text(
        ...
        ``strip_bot_mention=False`` to preserve the mention: that caller
        searches for the raw ``<@id>`` token, so ``strip_bot_mentions: false``
        must not rewrite it into ``@BotName`` here either.
...
def _apply_yaml_config(yaml_cfg: dict, slack_cfg: dict) -> dict | None:
    ...
    if bu is not None:
        ...
        if bu:
            extras["base_url"] = bu
    # Every other setting flows through env; only base_url is merged into extras.
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # the adapter reads via os.getenv(). The one non-env key, base_url, is
        # returned in extras and merged into PlatformConfig.extra. Replaces the
