...
- **CLI:** `hermes_cli/kanban.py` wires `hermes kanban` with verbs
  ...
  `request-review`, `reopen-review`, `block`, `unblock`, `archive`, `tail`,
  plus less-commonly-used `watch`, `stats`, `runs`, `log`, `assignees`,
...
- **Worker/orchestrator toolset:** `tools/kanban_tools.py` exposes
  ...
  explicitly enable the `kanban` toolset outside a dispatcher-spawned task
  also get `kanban_list` and `kanban_unblock` for board routing.
