        ...
        if _slack_dm_target.startswith(("user:", "user_name:")):
            ...
            _resolved, _resolve_err = _run_async(
                _resolve_slack_user_target(
                    pconfig.token,
                    _slack_dm_target,
                    base_url=_slack_dm_base_url(getattr(pconfig, "extra", None)),
                )
            )
            if _resolve_err:
...
async def _resolve_slack_user_target(token, chat_id, *, base_url=None):
    ...
    ``base_url`` overrides the Slack Web API endpoint (config.yaml
    ``platforms.slack.base_url``) so a self-hosted/mock Slack is resolved
    through the same endpoint the rest of the integration uses.
    ...
    try:
        ...
        async def post_api(session, method, payload):
            async with session.post(f"{api_base}{method}", headers=headers, json=payload, **_req_kw) as resp:
