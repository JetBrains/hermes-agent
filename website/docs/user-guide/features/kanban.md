...
The dispatcher refuses to re-spawn a ready task when it hit a quota/auth/429 error on the previous run (`blocker_auth`), or completed a run successfully within the guard window (`recent_success`), or a recent task comment links to a still-open GitHub PR after successful prior work (`active_pr`). This prevents repeat worker storms and naive duplicate respawns on the same task while a human catches up. See the `respawn_guarded` row in the [event reference](#event-reference).

The `active_pr` guard keeps **anti-duplicate** protection after a worker successfully finished work that already has a live PR. It only stops an **unintended** duplicate — it never blocks work you asked for on purpose, and it does not park a card that recovery already put back on `ready`. It steps aside automatically when:

- the **latest finished run is incomplete** — crash detection, orphan reconcile, stale-claim release, max-runtime timeout, reclaim, block, spawn failure, or gave-up left the run as `crashed` / `reclaimed` / `stale` / `timed_out` / `blocked` / `spawn_failed` / `gave_up`. The dispatcher already restored the task to `ready`; same-task continue is allowed even if a mid-run comment still links an open PR. A later **completed** run with the same open PR re-arms the guard;
...
To clear the guard by hand for a deliberate follow-up after **successful** PR work — without editing the database or deleting comments — use:

```bash
hermes kanban unguard t_abcd --reason "intentional rework after review"
```

`unguard` records a `respawn_guard_cleared` event that the next dispatcher tick treats as a fresh "continue this task" signal, so the worker re-spawns on the next tick. Incomplete recovery (crash/orphan/stale/timeout/reclaim) already continues automatically and does not require `unguard`. It doesn't change the task's status; if the task is `blocked`, use `unblock` instead (which already clears the guard as part of the transition). `/kanban unguard t_abcd` works from the gateway too.
...
| `respawn_guard_cleared` | `{actor?, reason?, cleared_through_comment_id?, pr_url?}` | An operator ran `hermes kanban unguard` to clear the `active_pr` respawn guard for a deliberate follow-up after successful PR work. Treated as a fresh continuation signal on the next tick; does not change status. Incomplete recovery (crash/orphan/stale/timeout/reclaim) already continues without this event. `cleared_through_comment_id` pins the PR comment the override was issued against, so the decision doesn't depend on whole-second timestamps — and a *new* PR link posted later re-arms the guard. |
...
| `respawn_guarded` | `{reason}` | Dispatcher refused to re-spawn this ready task this tick. Reasons: `blocker_auth` (last failure was a quota/auth/429 error — wait for the rate window to reset), `recent_success` (a completed run happened in the last hour — wait for review before re-running), `active_pr` (a prior successful run's still-open GitHub PR is referenced in a recent comment, with no newer deliberate follow-up / `unblock` / `unguard` signal and no incomplete latest-run recovery — see [Respawn guard](#respawn-guard) for how it yields to crash/orphan continue, intentional rework, never-run tasks and closed/merged PRs). The task stays in `ready`; the next tick gets another chance to spawn. If the underlying condition persists, the normal `consecutive_failures` circuit breaker will auto-block via `gave_up` after `failure_limit` failures. |
