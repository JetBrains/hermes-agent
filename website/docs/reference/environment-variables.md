...
| `SLACK_STRIP_BOT_MENTIONS` | Delete the bot's own @mention from the text the agent sees (default `true`). `false` keeps it, rendered as `@BotName`, so the agent can tell an explicit tag from a thread-routed wake-up; routing and commands are unaffected. Canonical setting: `slack.strip_bot_mentions` in `config.yaml`. |
